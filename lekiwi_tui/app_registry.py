...
class Action:
    ...
    kind: Kind
    handler: str
    card: str = ""  # menu card this action sits in; defaults to `section`. COLLECT splits
                    # into COLLECT (do it) + DATA (curate it) so the cards stay balanced.
    brief: str = ""  # short form of `hint` for the menu cards, which are half-width. Falls
                     # back to `hint`; the menu marks it with "…" if even this overflows.


# ── Action registry ───────────────────────────────────────────────────────────
ACTIONS: list[Action] = [
    Action("host-launch", "🚀", "Start host", "start the Pi robot host; choose session length", "HOST", "host", "lekiwi_tui.screens.host:HostLaunchScreen", brief="choose session length"),
    ...
    Action("edit-dataset", "📊", "Edit dataset", "pick one to edit episodes, or several to merge", "COLLECT", "screen", "lekiwi_tui.screens.dataset_select:DatasetSelectScreen", card="DATA", brief="edit episodes, or merge"),
    ...
    Action("train", "🧠", "Train", "fine-tune SmolVLA locally", "LEARN", "stream", "lekiwi_tui.screens.train:TrainScreen", brief="fine-tune SmolVLA locally"),
    Action("eval", "🦾", "Run policy", "run a trained policy without recording", "LEARN", "stream", "lekiwi_tui.screens.eval:EvalScreen", brief="run a policy, no recording"),
    ...
    Action("settings", "⚙", "Settings", "edit launcher settings in lekiwi.yaml", "SETUP", "screen", "lekiwi_tui.screens.settings:SettingsScreen"),
]
...
ALIASES: dict[str, str] = {
    ...
    "t": "teleop",
    "r": "record",
    "edit": "edit-dataset", "dataset": "edit-dataset", "merge": "edit-dataset",
    "d": "dagger", "hil": "dagger",
    "p": "replay",
    "viz": "view", "v": "view",
    "c": "calibrate",
